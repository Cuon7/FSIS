...
public class Query {
  ...
  public String toString() {
	 ...
	 * <pre>
	  ...
	  * @abstract_properties
          * none
          * index<MatchGen.size() /\ MatchGen.new =[x1....] /\

	 */
	public class MatchGen implements Iterator {
		 ...
		 * <pre>
		 * @effects
		 *  if ind >= matches.size() or matches.size() == 0
		 *      return false
		 *  else
		 *      return true
		 * @return
		...
		public boolean hasNext() {
			return (matches.size() != 0 && ind < matches.size());
		}
		...
		public Object next() {
			if (hasNext()) {
				Object next = matches.get(ind);
				ind++;
				return next;
			}

			throw new NotPossibleException("Query.MatchGen");
